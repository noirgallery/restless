...
const makeRPCMethodClient = (srcDir, path, name, rpcEndpoint) => {
    ...
    return fetch(${JSON.stringify(rpcEndpoint)}, { method: "POST", credentials: "include", headers: {"Content-Type": "application/json"},  body: JSON.stringify({ id: ${JSON.stringify(methodId)}, args: params }) }).then(res => { 
    if (res.status === 200) {
      return res.json()
    } else {
...
function default_1(content, map, meta) {
    ...
    return names
        .map((value) => makeRPCMethodClient(srcDir, this.resourcePath.replace(serverPattern, ""), value, endpoint))
        .join("\n");
}
