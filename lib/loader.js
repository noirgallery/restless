...
const makeRPCMethodClient = (srcDir, path, name) => {
    ...
    return fetch(${JSON.stringify(RPC_HOST + "/__rpc")}, { method: "POST", headers: {"Content-Type": "application/json"},  body: JSON.stringify({ id: ${JSON.stringify(methodId)}, args: params }) }).then(res => { 
    if (res.status === 200) {
      return res.json()
    } else {
